...
def _javascript_analysis(source: str, language: str) -> dict[str, Any]:
    ...
    return {
        ...
        "classes": classes[:120],
        "syntax_error": None,
        "note": f"{language} はv2.2で宣言ベースの軽量解析です。メソッド呼び出しを関数定義として数えないようにしています。",
    }
...
def _css_analysis(source: str, language: str) -> dict[str, Any]:
    ...
    return {
        ...
        "syntax_error": None,
        "note": f"{language} はv2.2でCSS専用の軽量解析です。セレクタ/@規則/CSS変数を取得し、@mediaを関数として誤検出しません。",
    }
...
def _html_analysis(source: str) -> dict[str, Any]:
    ...
    return {
        ...
        "syntax_error": None,
        "note": "HTML はv2.2でscript/link/img等の参照とid/classを軽量解析します。",
    }
...
def _lightweight_analysis(source: str, language: str) -> dict[str, Any]:
    ...
    return {
        ...
        "classes": classes[:100],
        "syntax_error": None,
        "note": f"{language} はv2.2では軽量な宣言パターン解析です。意味解析はOllamaが補完します。",
    }
